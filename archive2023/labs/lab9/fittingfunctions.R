fitLogistic <- function(data, y = "N", time = "Day", N0 = 1, K = 200, r0 = 0.75){
  ...
  myfit <- nls(Y ~ N.logistic(X, N0, K, r0),  
...
simPlot <- function(fit, data = NULL, species1 = "N1", species2 = "N2"){
  ...
  legend("topleft", legend = c(species1, species2), lwd = 3, col = cols)
}
...
getR2 <- function(fit, data, species1, species2){
  ...
  frommean <- cbind(N1 = data[,species1] - mean(data[,species1], na.rm = TRUE),
                    N2 = data[,species2] - mean(data[,species2], na.rm = TRUE))
  
  rSS <- apply(residuals, 2, function(x) sum(x^2, na.rm = TRUE))
  SST <- apply(frommean, 2, function(x) sum(x^2, na.rm = TRUE))
