...
demographicstochasticity <- function(t, N0, b.prob, d.prob, N.sims,
  ...
  pop.high <- apply(Sims,1, quantile, .75)
...
server <- function(input, output) {
   sim <- eventReactive(input$go,
    ...
    output$timeseries <- renderPlot({
        ...
        par(mfrow = c(1,2), tck = 0.01, bty = "l", cex.lab = 1.5, mgp = c(1.5,.25,0))
        ...
        matplot(0:t, sim()$simulation, type = "l", lty = 1, col = rgb(0,0,0,.3), 
                xlab = "Time", ylab = "Population", log = "y")
    ...
    output$survivalreport <- renderText({
      paste0("Final population - median: ", round(median(sim()$final.pop),2), 
            ", inter-quartile range: ", round(quantile(sim()$final.pop, .25),2), " - ", 
            round(quantile(sim()$final.pop, .75),2))
    })
    
    output$extinctionreport <- renderText({
      paste0("Time to extinction - mean: ", round(mean(sim()$extinctions),3), ", standard deviation:", round(sd(sim()$extinction), 3))
    })
}
