...
Isoclines <- function(competition, predation, rv, rp, K, xlim, n.arrows){
  ...
  z.grid <- outer(xlim*seq(0,1.4,length = n.arrows)[-1], 
                  xlim*seq(0,1.4,length = n.arrows)[-1], 
                  function(x,y) x + 1i*y) 
  ...
  #arrow.plot  <- complex(arg = Arg(arrow), mod = sqrt(Mod(arrow)))
