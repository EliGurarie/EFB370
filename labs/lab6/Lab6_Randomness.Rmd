...
author: "Dr. Gurarie and Katya Khadonova"
date: "`r Sys.Date()`"
output: 
  html_document: 
    toc: true
...
> There are TWO exercises in this lab, set aside in green boxes like this one.  To work through the exercises in this lab, you'll need to download the [`SeaLions.csv`](../data/SeaLions.csv) and [`SeaOtters.csv`](../data/SeaOtters.csv) datasets as last week by following the links (also available on Blackboard). For Exercise 2, you will also need to download [`BC_SeaOtters.csv`](../data/BC_SeaOtters.csv). If you've forgotten how, follow the instructions from [Lab 5](../lab5/Lab5_IntroToR.Rmd). 

```{r}
SeaOtters <- read.csv("../data/SeaOtters.csv")
SeaLions <- read.csv("../data/SeaLions.csv")
```
...
>
> For the lab exercise **all you need to do** is (1) Load the data, (2) plot the data, and (3) report how many were released, in what year, how many are there in the final year of data.  
>
...
> Report the **probability of extinction due to demographic stochasticity** if only **2**, **10** and **60** sea otters were released.
>
> 2. In the **Environmental Stochasticity** simulater, set the **mean population growth rate** ($\mu_r$) to 0.07, the **standard deviation of growth rate** ($\sigma_r$) to 0.03, the $t_{max}$ to 40 and the number of simulations to 1000. Again, this roughly simulates sea otter population growth.  
> Again, report the **probability of extinction due to environmental stochasticity** if only **2**, **10** and **60** sea otters were released