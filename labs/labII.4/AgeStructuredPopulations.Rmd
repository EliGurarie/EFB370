...
subtitle: '[EFB 370](https://eligurarie.github.io/EFB370/) - Spring 2025'
author: "E. Gurarie"
date: "March 5, 2026"
output: 
  html_document: 
...
> There are THREE exercises in this lab to submit by midnight Friday, March 6.  
...
Download and open the "Life Tables" Excel file from the Recitations folder on Blackboard or [here](LifeTables_EFB370.xlsx).
...
leslie_diagram(M, stages = c("egg", "caterpillar", "adult"))
```
...
Pre infectious facial tumor disease, the life history population matrix for Tasmanian devils looked something like this:
...
>
>
> After facial tumor disease emerged and established itself in the Tasmanian devil population, annual mortality of 3-plus year olds skyrocketed to 90%. This had an immediate impact on the population growth factor $\lambda$.   
> 
...
>
> 2. (**4 pts.**)  Using the Matrix Population Simulator, run the post-disease population for 10 years starting from the pre-disease stable age distribution N∗N∗ you found in Exercise 4. Copy and paste the resulting figure directly from the simulator into your document.
> 
> 3. **3. (6 pts.) Sensitivity analysis and conservation priorities**

> The matrix has six non-zero entries. Increase each one by 10% (one at a time, returning to the disease-scenario values between each trial), record the resulting $\lambda$, and fill in a table like this:

| Parameter | Original value | +10% value | $\lambda$ (original) | $\lambda$ (modified) | Change in $\lambda$ |
|---|---|---|---|---|---|
| $F_1$ (yr-1 fecundity) | | | | | |
| $F_2$ (yr-2 fecundity) | | | | | |
| $F_{3+}$ (yr-3+ fecundity) | | | | | |
...
> Which parameter has the largest effect on $\lambda$? Which has the smallest? Given that conservation resources are limited, where would you focus intervention efforts — and does your answer change depending on whether the intervention is *easier to achieve biologically* (e.g., reducing disease transmission to older animals vs. increasing birth rates in yearlings)?
> 
> Describe in 2–3 sentences what you see: Is the population declining smoothly, or is there a transient phase before settling into a steady trajectory? 

> A few notes on these drafts: the sensitivity table assumes all six non-zero entries are varied — you may want to fix g3+g3+​ (the self-loop survival) as a separate case since it's structurally different from the others. Also, the transient dynamics question is genuinely interesting here because starting from the pre-disease N∗N∗ (older-skewed) will produce a visible "cliff" before settling — worth flagging to students in a hint if you want them to notice it rather than just describe a smooth decline.



Here's a draft for two new homework questions:
...
**4. (4 pts.) Population trajectory**

Using the Matrix Population Simulator, run the post-disease population for **20 years** starting from the pre-disease stable age distribution $N^*$ you found in Exercise 4. Copy and paste the resulting figure directly from the simulator into your document.

Describe in 2–3 sentences what you see: Is the population declining smoothly, or is there a transient phase before settling into a steady trajectory? What does this tell you about the short-term versus long-term risk to the population?

---

A few notes on these drafts: the sensitivity table assumes all six non-zero entries are varied — you may want to fix $g_{3+}$ (the self-loop survival) as a separate case since it's structurally different from the others. Also, the transient dynamics question is genuinely interesting here because starting from the pre-disease $N^*$ (older-skewed) will produce a visible "cliff" before settling — worth flagging to students in a hint if you want them to notice it rather than just describe a smooth decline.




> 2. (**4 pts.**) As a population-level response, Tasmanian devils radically increased the number of "precocial" births, i.e. 1 yr olds that reproduce.  How high must 1 year old birth rates become to rescue the Tasmanian devil population?   
> 
> 
> 
