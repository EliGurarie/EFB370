...
- The overal population growth rate is then $r = log(R_0) / G$. **NOTE:** In Excel, the *natutal log* (base e) function is `=ln()`, unlike R, where it's just `log()`.
...
After facial tumor disease emerged and established itself in the Tasmanian devil population, annual mortality of 2 year old and 3-plus year olds skyrocketed to 90%. This had an immediate impact on the population growth factor $\lambda$.   


> **Exercise 5:**
> 
> What are the new values for $\lambda$ and $N^{*}$?  (This is also part of problem 1 on the HW below). 
...
M.post <- rbind(c(0,.8,.7,.7),
               c(.7,0,0,0),
               c(0,.8,0,0),
               c(0,0,.1,.1))
eigen(M.post)
(eigen(M.pre)$vectors[,1] / sum(eigen(M.pre)$vectors[,1]) * 100) |> round(1)
(eigen(M.post)$vectors[,1] / sum(eigen(M.post)$vectors[,1]) * 100) |> round(1)
```
...
> 
> 1. (**2 pts.**) Changing only those survival probabilities, write a new population matrix $M_1$. Report the new population growth factor and the new stable age distribution?  Using the **Matrix Population Simulator**, run the post-disease population for 10 years starting from the pre-disease stable age distribution N∗N∗ you found in Exercise 4. Copy and paste the resulting figure directly from the simulator into your document. What are the population level consequences of increased mortality of higher age classes? 
>
> 2. (**2 pts.**) As a (fascinating) population-level response, Tasmanian devils radically increased the number of "precocial" births, i.e. 1 yr olds that reproduce.  Also, the 2 year old fecundity increases.  Report the new population matrix $M_2$ if both 1 year old nad 2 year old fecundities increased to 3+ year old fecundity (which we can assume is a kind of ceiling).  Would that be enough to rescue the population?  Explain why or why not. Plot the trajectories of the 4 age group over 10 years (as above - using the **Matrix Population Simulator**)  
> 
> 3. (**6 pts.**) The population matrix $M_2$ (from question 2 has seven non-zero entries - 3 fecundites, and 4 survivals. Increase each of those values by 10% (one at a time, returning to the disease-scenario values between each trial), record the resulting $\lambda$, and fill in a table like this:
> 
> | Parameter | Original value | +10% value | $\lambda_0$ (original) | $\lambda_1$ (modified) |   $\lambda_1/\lambda_0$ |
>|---|---|---|---|---|---|
>| $f_1$ (yr-1) | | | | | |
>| $f_2$ (yr-2) | | | | | |
>| $f_{3+}$ (yr-3+) | | | | | |
...
>| $s_{3+}$ (yr-3+) | | | | | |
>
> Which parameter has the largest effect on $\lambda$? Which has the smallest? Given that conservation resources are limited, where would you focus intervention efforts?  Consider both the sensitivity of $\lambda$ to the paramater, and the feasibility of the intervention. 
...
**Is this Gen Z humor?  If so, I don't get it. But kudos for accuracy.**
