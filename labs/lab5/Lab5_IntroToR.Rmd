...
This dataset is available on Blackboard. Once you download it, you can use the File Explorer to determine its location and read it into R. You can do it in a couple of ways. First, you can modify and run this line of code:
...
This method does the same exact thing as the line of code above. It will automatically input the proper code into the console and save your file to the environment. Note that the file has the same name rather than a name you designate for it.
...
```{r, echo = -1}
par(mfrow = c(1,2)) #do not worry about this line - this simply puts two graphs side-by-side
hist(Length)
hist(Weight)
```
...
## Exercise 2: Download and plot the sea otter population growth

### Importing Data

> The first step is to import our new dataset. Perform the same steps as we did with the sea lions one 
...
> Plot the downloaded frame, using the "Year" column on the X-axis and "Count" column on the y-axis. Hint: you will need to extract these columns before plotting.
