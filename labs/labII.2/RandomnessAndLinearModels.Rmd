...
> There are THREE exercises in this lab, set aside in green boxes like this one. Two involve R, and one involves a seperate shiny app (link below) To work through Exercise 1, you'll need the same files as last week: [`SeaLions.csv`](SeaLions.csv) and [`SeaOtters.csv`](SeaOtters.csv) (also available on Blackboard). For Exercise 2, you will also need to download [`BC_SeaOtters.csv`](BC_SeaOtters.csv). 

>**FOR SUBMISSION** Please Upload a word document with answers to:
*Exercise 1: parts 1-3;  Exercise 2: part 3;  Exercise 3: parts 1 and 2* in the Recitation 5 (R pt 2) on  **Blackboard** by midnight, Friday, Feb 20.  
...
> # Lab Exercise 3
>
> The final exercise is NOT in R!  It is just some practice experimenting with the stochastic population simulator here: https://egurarie.shinyapps.io/StochasticGrowth.  The goal will be to see how sensitive the sea otter success is to the initial population released. 
> 
...
>
> 2. In the **Environmental Stochasticity** simulater, set the **mean population growth rate** ($\mu_r$) to **7%** (i.e. 0.07), the **environmental stochasticity** ($\sigma_r$) to 0.01, the $t_{max}$ to 40, the **number of otters released** ($N_0$) to 60, and the number of simulations to 1000. Again, this (very) roughly simulates sea otter population growth.  
