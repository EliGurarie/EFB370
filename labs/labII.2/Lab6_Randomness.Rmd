...
Exercise 1: parts 1-3;  Exercise 2: part 3;  Exercise 3: parts 1 and 2.

For importing data "best practice" would be to create a new R project in its own file folder and then move data files (.csv's) into that folder. R looks for files in it's project's directory. This is your projects whole world. If you ask it to find a file located in 'Downloads', R will say something like "***no such file exists in working directory***". As a result, the following code will not run unless you put the .csv files in your project directory. I (Colton) have to run this code to get R to write this document. **You** should skip this chunk and follow the instructions at "**HERE**"
```{r}
#This is where your R project is looking for files (working directory)
getwd()

#copy and paste the file path into list.files. Tells R to list all the .csv files in this directory 
list.files(path="C:/Users/colto/Teaching/EFB370/labs/labII.2", pattern=".csv", all.files=TRUE, full.names=TRUE)

#If you have not changed the file directory from downloading above, your files are in your downloads folder.

#each time you download the file, your computer will give it a new name: SeaOtters.csv, SeaOtters(1).csv, SeaOtters(2).csv, etc

#This names the file 'SeaOtters.csv' as the object 'SeaOtters'
#So when you refer to 'SeaOtters' it knows this dataframe

#Run This:
 SeaLions <- read.csv("C:/Users/colto/Teaching/EFB370/labs/labII.2/SeaLions.csv")
 SeaOtters <- read.csv("C:/Users/colto/Teaching/EFB370/labs/labII.2/SeaOtters.csv")
```

**HERE** An alternative way to import data is by going to `File>Import Dataset>From Text (base)` and navigate to the the files in your library (they are probably in your downloads folder). Then click `Import`. 
